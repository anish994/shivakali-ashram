...
def replace_content(template, nakshatra):
    ...
    # First do a case-sensitive replace for exact matches
    replacements = {
        'ROHINI': nakshatra['name'].upper(),  # For the main title which is in caps
        'Rohini': nakshatra['name'],         # For other places with proper case
...
def generate_essence_html(essence_data):
    """Generate HTML for essence section from JSON data"""
    html = f'''
    ...
    for card in essence_data['cards']:
        html += f'''
            <div class="content-card expandable-card" onclick="toggleExpandableCard(this)">
                ...
                <div class="basic-content">{card['content']}</div>
                <div class="detailed-content">
                    {card.get('details', '')}
                </div>
            </div>
...
def replace_section_content(template, section_marker, new_content):
    ...
    if section_marker == 'Mythological Foundation':
        ...
        # Remove any existing Related Nakshatras sections first
        clean_template = template
        related_marker = '<h2 class="section-title">Explore Related Nakshatras</h2>'
        while True:
            idx = clean_template.find(related_marker)
            if idx == -1:
                break
            section_start = clean_template.rfind('<section', 0, idx)
            section_end = clean_template.find('</section>', idx) + len('</section>')
            if section_start == -1 or section_end == -1:
                break
            clean_template = clean_template[:section_start] + clean_template[section_end:]
        
        # Generate HTML for both essence and related sections
        essence_html = generate_essence_html(new_content)
        related_html = generate_related_nakshatras_html(new_content.get('related_nakshatras', {}))
        combined_html = f'''{essence_html}
</div>
</section>

<!-- Related Nakshatras Section -->
<section class="content-section">
{related_html}'''
        
        # Find and replace the essence section
        section_marker = '<h2 class="section-title">The Essence of'
        start_idx = clean_template.find(section_marker)
        if start_idx == -1:
            return template
        section_start = clean_template.rfind('<div class="two-column-grid">', 0, start_idx)
        section_end = clean_template.find('</section>', start_idx) + len('</section>')
        if section_start == -1 or section_end == -1:
            return template
            
        return clean_template[:section_start] + combined_html + clean_template[section_end:]
    else:
        content_html = str(new_content)
    
    # For all other sections, simple replacement between content markers
    ...
    section_end = template.find('</section>', section_start)
    if section_start == -1 or section_end == -1:
        return template
